...
var comNames = ['IKEA', 'IBM','Porsche','Moët et Chandon','Audi','Burberry','Ralph Lauren Corporation','Deere & Company','Louis Vuitton','Wells Fargo','Adidas','Bank of America','Smirnoff','Hyundai','Johnson & Johnson','Sprite','Heineken Brewery','Prada','Google','BMW','Avon','PepsiCo','Starbucks','Samsung Group',"McDonald's"];
var animalNames = ['lizards', 'deers', 'chipmunks', 'chimpanzees', 'hogs', 'dogs', 'cats', 'eagles'];

for(var i = 0; i < 5; i++)
{
	console.log(comNames[Math.floor(Math.random()*25)] + ' is a company that makes sweaters for ' + animalNames[(Math.floor(Math.random()*8))]);
}